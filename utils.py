...
def read_parameters():
    ...
    # input file location
    parser.add_argument("--model_location",type=str,default="bert-base-cased/")
    ...
    parser.add_argument('--context_embed_dim', type=int, default=200)

    parser.add_argument("--context_learning_rate",type=float,default=2e-3)
...
def train(model,train_dataloader,device,gradient_accumulation_steps,optimizer,lr_scheduler,progress_bar):
    model.train()
    print("\n")
...
def validate(model, valid_dataloader, device,label_list):
    model.eval()
    raw_predicts = []
    raw_labels = []
    with torch.no_grad():
        for batch in valid_dataloader:
            ...
            for i in range(len(label_ids)):
                process_label_ids.append(np.delete(label_ids[i],np.where(label_ids[i]<0)))
            raw_predicts.append(predicts)
            raw_labels.append(process_label_ids)
...
def test(model,eval_dataloader,device,label_list):
    model.eval()
    raw_predicts = []
    raw_labels = []
    with torch.no_grad():
    ...
    return API_Precision, API_Recall, API_F1
